...
import { GetTreatment, UpdateSchedule, GetScheduleById } from "../../../services/https/payment";
...
function ScheduleEdit() {
  ...
  const onFinish = async (values: SchedulesInterface) => {
    ...
    try {
      let res = await UpdateSchedule(values); // ใช้ฟังก์ชัน UpdateSchedule แทน CreateSchedule
      if (res.status) {
        messageApi.open({
          type: "error",
...
//   const onFinish = async (values: SchedulesInterface) => {
//     // ตรวจสอบว่า schedule มีค่าอยู่หรือไม่
//     //if (!schedule) return;
//     //values.ID = schedule?.ID;
//     // รวมค่าที่มีการอัปเดตกับค่าที่ไม่ได้กรอก
...
//     try {
//         let res = await UpdateSchedule(updatedValues); // ใช้ฟังก์ชัน UpdateSchedule
//         if (res.status) {
//             messageApi.open({
//                 type: "success",
//                 content: "อัปเดตข้อมูลสำเร็จ",
